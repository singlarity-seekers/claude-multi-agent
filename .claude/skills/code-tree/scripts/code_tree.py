        ...
        if sym_type is not None:
            ...
            if name:
                ...
                if actual_type is None:
                    # Skip scope-only nodes (like impl blocks)
                    for child in node.children:
                        visit(child, scope=name, parent_id=parent_id)
                    return
...
def resolve_imports(
    ...
    # Build class/struct/interface name -> symbol for O(1) inheritance resolution
    class_by_name = {}
    for sym in symbols:
    ...
    for edge in resolved:
        if edge.type == "inherits" and edge.metadata.get("unresolved"):
            ...
            base_sym = class_by_name.get(base_name)
            if base_sym:
                final.append(Edge(edge.source, base_sym.id, "inherits"))
            else:
                final.append(edge)
        else:
            final.append(edge)
...
def generate_summary_md(
    ...
    for d in sorted(dir_stats.keys()):
        ...
        test_str = f", {stats['test_files']} tests" if stats.get("test_files") else ""
        lines.append(f"{indent}{dir_name}/ ({stats['files']} files{test_str}, {lang_str})")
    lines.append("```\n")
...
def main():
    ...
    parser.add_argument(
        "--include-tests", action="store_true", default=True,
        help="Include test files (now default; kept for compatibility)",
    )
    parser.add_argument(
